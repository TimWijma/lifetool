...
export const useWindowStore = defineStore('windows', () => {
    ...
    try {
      ...
      if (stored) {
        ...
        Object.entries(parsed).forEach(([type, state]) => {
          ...
          maxZIndex.value = Math.max(maxZIndex.value, windowState.zIndex + 1)
        })
      }
    } catch (error) {
      console.warn('Failed to load window states from localStorage:', error)
    }
  }
  ...
  const saveWindowStates = () => {
    try {
      ...
      localStorage.setItem('lifetool-windows', JSON.stringify(dataToSave))
    } catch (error) {
      console.warn('Failed to save window states to localStorage:', error)
    }
  }
  ...
  const updateWindowPosition = (type: WindowType, position: { x: number; y: number }) => {
    const windowState = windows.value.get(type)
  ...
  const updateWindowSize = (type: WindowType, size: { width: number; height: number }, config: WindowConfig) => {
    const windowState = windows.value.get(type)
    if (!windowState) return
