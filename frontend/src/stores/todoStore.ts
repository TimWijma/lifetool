...
export const useTodoStore = defineStore("todos", () => {
    ...
    const allCompleted = computed(
        () =>
            ...
            completedCount.value === filteredTodos.value.length
    );
    ...
    const addTodo = (text: string, tags: string[] = []) => {
        ...
        if (trimmed.length > 200)
            throw new Error("Todo cannot exceed 200 characters");
        ...
        const finalTags =
            tags.length > 0
                ? tags
                : activeTag.value !== "all" && activeTag.value !== "untagged"
                ? [activeTag.value]
                : [];
    ...
    const clearCompleted = () => {
        ...
        if (activeTag.value === "all") {
            ...
            todos.value = todos.value.filter(
                (todo) => !todo.completed || todo.tags.length > 0
            );
        } else {
            todos.value = todos.value.filter(
                (todo) =>
                    !todo.completed || !todo.tags.includes(activeTag.value)
            );
        }
        saveTodos();
    };
